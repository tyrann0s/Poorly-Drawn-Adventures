...
#if UNITY_EDITOR
[CustomPropertyDrawer(typeof(ActiveSkill), true)]
public class ActiveSkillDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        EditorGUI.BeginProperty(position, label, property);

        // Получаем все типы, наследующиеся от ActiveSkill
        var types = AppDomain.CurrentDomain.GetAssemblies()
            .SelectMany(s => s.GetTypes())
            .Where(p => typeof(ActiveSkill).IsAssignableFrom(p) && !p.IsAbstract)
            .ToArray();
